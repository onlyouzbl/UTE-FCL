...
class Recipe1M(Dataset):
    ...
    def __init__(self, root, transform=None, split='train',
        ...
        #load vocabulary
        # vocab_inv ---   {0: '<pad>', 1: '<start>', 2: '<end>', ...... , 16302: '<unk>'}
    ...
    def __getitem__(self, idx):
        ...
        instrs = [get_token_ids(instr, self.vocab)[:self.max_length_instrs] for instr in instrs[:self.max_instrs]]    
        for instr in instrs:
            recipe.extend(instr)
            instrs_whole.extend(instr)
       
        ingrs = [get_token_ids(ingr, self.vocab)[:self.max_length_ingrs] for ingr in ingrs[:self.max_ingrs]]   
        for ingr in ingrs:
            recipe.extend(ingr)
